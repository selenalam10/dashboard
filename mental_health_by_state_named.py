...
# Load BRFSS dataset
file_path = "C:/Users/selen/OneDrive/Desktop/mental health project/LLCP2022.XPT"
df = pd.read_sas(file_path)

# Keep only the necessary columns
df = df[['_STATE', 'ADDEPEV3', 'MENTHLTH']]
df_clean = df.dropna()

# Depression % per state
depression_by_state = (
    df_clean.groupby('_STATE')['ADDEPEV3']
    .apply(lambda x: (x == 1).mean() * 100)
    .round(2)
    .reset_index(name='Depression_Pct')
)

# Avg mental health days per state
mental_days_by_state = (
    df_clean.groupby('_STATE')['MENTHLTH']
    .mean()
    .round(2)
...
# Map FIPS codes to full state names
fips_to_state = {
...
# Print and save
print(summary)
summary.to_csv("mental_health_by_state_named.csv", index=False)
